...
namespace details {
template <class L, class func_addr_type, class R, class... Args>
func_addr_type impl_impl(L l) {
...
struct to_f_impl<R (ClassType::*)(Args...) const> {
  using func_addr_type = R (*)(Args...);

  template <class L> static func_addr_type impl(L l) {
    return impl_impl<L, func_addr_type, R, Args...>(std::move(l));
  }
};
template <class ClassType, class R, class... Args>
...
template <class L> typename details::to_f_impl<L>::func_addr_type to_f(L l) {
